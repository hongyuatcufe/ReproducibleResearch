...
filling in all of the missing values in the dataset with mean of steps of relevant day and Create a new dataset

```{r echo=TRUE}
activity_changed = activity %>%
  merge(.,steps_per_day,by="date") %>%
  # every day has 288 intervals, it could be caculate as: 24(hours) * 60(minutes per hour) / 5(minutes per interval)
  # mean of every interval of day could be caculate as: total steps of that day / 288 intervals
  mutate(steps.x=ifelse(is.na(steps.x),steps.y/288,steps.x)) %>%
...
head(activity_changed[is.na(activity$steps),])
```
